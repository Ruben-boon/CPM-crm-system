...
export function MultiRefField({
  ...
  useEffect(() => {
    const hasValueChanges = JSON.stringify(value) !== JSON.stringify(previousValuesRef.current);
  ...
  const handleQuickAdd = () => {
    if (collectionName === "contacts" && openModal) {
      openModal("contact", {
        initialData: {
          general: { role: "guest" }
        },
  ...
  return (
    <div className="multi-ref-field">
      ...
      <div className={`ref-field-container ${isChanged ? "field-changed" : ""}`} ref={dropdownRef}>
